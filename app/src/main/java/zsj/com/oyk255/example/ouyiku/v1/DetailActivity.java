...
//				View topView = getLayoutInflater().inflate(R.layout.detail_topview, null);
    ...
    private void initData() {
        ...
        progressHUD.show();
        //详情页数据
        String DetaiBannerUrl = Constant.URL.DetailBannerURL;
                        ...
                        public void onClick(View v) {
                            if (aaa.length > 0) {
                                for (int i = 0; i < aaa.length; i++) {
                                    if (aaa[i] == null || aaa[i].equals("null,") || aaa[i].equals("null")) {
                                        Toast.makeText(DetailActivity.this, "请选择正确的属性", Toast.LENGTH_SHORT).show();
                                        return;
                                    }
                                }
                            ...
                            for (int i = 0; i < mStockIdData.size(); i++) {
                                ...
                                if (a.toString().equals(mStockIdData.get(i))) {
                                    ...
                                    final ZProgressHUD progressHUD = ZProgressHUD.getInstance(DetailActivity.this);
                                    progressHUD.setMessage("加载中");
                                    progressHUD.setSpinnerType(ZProgressHUD.SIMPLE_ROUND_SPINNER);
                                    progressHUD.show();
                                    ...
                                    map.put("token", token);
                                    Log.e("use_id", use_id);
                                    HTTPUtils.post(DetailActivity.this, url, map, new VolleyListener() {
                                        ...
                                        public void onResponse(String arg0) {
                                            ...
                                            if (status.getSucceed().equals("1")) {
                                                ...
                                                Toast.makeText(DetailActivity.this, "加入购物车失败", Toast.LENGTH_SHORT).show();
                                            }
                                        }
                                    ...
                                    HTTPUtils.post(DetailActivity.this, url, map, new VolleyListener() {
                                        ...
                                        public void onResponse(String arg0) {
                                            ...
                                            if (status.getSucceed().equals("1")) {
                                                IntoShopCarData data = fromJson.getData();
                                                String cartId = data.getCartId();
                                                Intent intent3 = new Intent(DetailActivity.this, ConfirmOrderActivity.class);
...
//            Toast.makeText(DetailActivity.this, "onError: " + e.errorMessage, Toast.LENGTH_SHORT).show();
    ...
    private void doShareToQzone(final Bundle params) {
        ...
        ThreadManager.getMainHandler().post(new Runnable() {
            ...
            public void run() {
                if (null != mTencent) {
                    mTencent.shareToQzone(DetailActivity.this, params, qqShareListener);
                }
            }
        });
    }
