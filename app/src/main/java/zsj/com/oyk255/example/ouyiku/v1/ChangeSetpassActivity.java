...
public class ChangeSetpassActivity extends OykActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_change_setpass);
...
private void SetTiXianPass(String pass){
	...
	progressHUD.show();
		String url= Constant.URL.SetNewsLoginUrl;
		...
		HTTPUtils.post(this, url, map, new VolleyListener() {
			...
			public void onResponse(String arg0) {
				...
				if(status.getSucceed().equals("1")){
					Toast.makeText(ChangeSetpassActivity.this, "设置成功", Toast.LENGTH_LONG).show();
					 finish();
				}
			}
	...
	public void onClick(View v) {
		...
		case R.id.tixiansetpass_finish:
			...
			if(firstPass.equals("")&&secondPass.equals("")){
				Toast.makeText(ChangeSetpassActivity.this, "密码不能为空", Toast.LENGTH_LONG).show();
			}else{
				if(!firstPass.equals(secondPass)){
					Toast.makeText(ChangeSetpassActivity.this, "两次输入密码不一样", Toast.LENGTH_LONG).show();
				}else{
