...
import zsj.com.oyk255.example.ouyiku.v1.Order_detail_daifuActivity;
import zsj.com.oyk255.example.ouyiku.v1.Order_detail_daishouActivity;
import zsj.com.oyk255.example.ouyiku.v1.PayActivity;
import zsj.com.oyk255.example.ouyiku.v1.WuliuActivity;
 ...
 *
 */
public class Order_allFragment extends Fragment implements OnClickListener{
	...
	private void deleteOrder(String orderId){
    	...
    	progressHUD.show();
		HTTPUtils.get(getActivity(), deleteUrl, new VolleyListener() {
	...
	private void initData() {
    	...
    	progressHUD.setSpinnerType(ZProgressHUD.SIMPLE_ROUND_SPINNER );
    	progressHUD.show();
		String AllOrderUrl=Constant.URL.MyOrderURL+"&user_id="+userid+"&status="+0;
		HTTPUtils.get(getActivity(), AllOrderUrl, new VolleyListener() {
			...
			public void onResponse(String arg0) {
				...
				if(succeed.equals("1")){
					...
					if(mData.size()==0){
						...
						allorderAdapter = new AllorderAdapter();
	...
	private void initUI() {
		mListview = (ListView) view.findViewById(R.id.order_all_listview);
		mImgNull = (ImageView) view.findViewById(R.id.order_all_null);
		...
		mListview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if(status.equals("1")){
					Intent intent = new  Intent(getActivity(), Order_detail_daifuActivity.class);
					...
					Intent intent = new  Intent(getActivity(), Order_detail_daishouActivity.class);
					intent.putExtra("orderId", orderId);
					startActivity(intent);
				}
			}
	...
	class AllorderAdapter extends BaseAdapter{
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if(convertView==null){
				layout=getActivity().getLayoutInflater().inflate(R.layout.order_all_item, null);
				...
				holder.order_goods_delect = (TextView) layout.findViewById(R.id.order_goods_delete);
				holder.order_goods_tobrand = (LinearLayout) layout.findViewById(R.id.order_tobrand);
				holder.mDeleteLayout = (LinearLayout) layout.findViewById(R.id.all_delete);
				holder.mAll_tixing = (LinearLayout) layout.findViewById(R.id.all_tixing);
				holder.mAll_daifu = (LinearLayout) layout.findViewById(R.id.all_daifu);
				holder.mAll_queren = (LinearLayout) layout.findViewById(R.id.all_queren);
				
				holder.mTixing = (TextView) layout.findViewById(R.id.order_goods_tixing);
				holder.mFukuan = (TextView) layout.findViewById(R.id.order_goods_fukuan);
				holder.mDelete_fukuan = (TextView) layout.findViewById(R.id.order_goods_delete);
				holder.mShouhuo = (TextView) layout.findViewById(R.id.order_goods_shouhuo);
				holder.mWuLiu = (TextView) layout.findViewById(R.id.order_goods_wuliu);
				...
				final String price = myOrderDatum.getPrice();//总价
				List<Good> goods = myOrderDatum.getGoods();
				Good good = goods.get(0);
				final String orderId = myOrderDatum.getOrderId();
				final String exphone = myOrderDatum.getExphone();
				final String expressnum = myOrderDatum.getExpressnum();
				final String expressname = myOrderDatum.getExpressname();
				final String ordernum = myOrderDatum.getOrdernum();
				...
				String status = myOrderDatum.getStatus();
				if(status.equals("1")){
					holder.mAll_daifu.setVisibility(View.VISIBLE);
					holder.mAll_tixing.setVisibility(View.GONE);
					holder.mDeleteLayout.setVisibility(View.GONE);
					holder.mAll_queren.setVisibility(View.GONE);
				}else if(status.equals("2")){
					holder.mAll_daifu.setVisibility(View.GONE);
					holder.mAll_tixing.setVisibility(View.VISIBLE);
					holder.mDeleteLayout.setVisibility(View.GONE);
					holder.mAll_queren.setVisibility(View.GONE);
					...
					holder.mAll_tixing.setVisibility(View.GONE);
					holder.mDeleteLayout.setVisibility(View.GONE);
					holder.mAll_queren.setVisibility(View.VISIBLE);
				}else{
					holder.mAll_daifu.setVisibility(View.GONE);
					holder.mAll_tixing.setVisibility(View.GONE);
				...
				holder.order_goods_delect.setOnClickListener(new OnClickListener() {
				...
				holder.order_goods_tobrand.setOnClickListener(new OnClickListener() {
				...
				holder.mTixing.setOnClickListener(new OnClickListener() {
				...
				holder.mDelete_fukuan.setOnClickListener(new OnClickListener() {
				...
				holder.mShouhuo.setOnClickListener(new OnClickListener() {
				...
				holder.mWuLiu.setOnClickListener(new OnClickListener() {
				...
				holder.mFukuan.setOnClickListener(new OnClickListener() {
					
					@Override
					public void onClick(View v) {
						Intent intent = new Intent(getActivity(), PayActivity.class);
						intent.putExtra("oids", orderId);
	...
	class ViewHolder{
		...
		TextView order_goods_delect;
		LinearLayout order_goods_tobrand;
		
		LinearLayout mDeleteLayout;
		LinearLayout mAll_tixing;
		LinearLayout mAll_daifu;
		LinearLayout mAll_queren;
		
		TextView mTixing;
		TextView mFukuan;
		TextView mDelete_fukuan;
		TextView mShouhuo;
		TextView mWuLiu;
	}
	
	private void CheckDelivery(String orderId){
		final ZProgressHUD progressHUD = ZProgressHUD.getInstance(getActivity()); 
    	progressHUD.setMessage("加载中");
    	progressHUD.setSpinnerType(ZProgressHUD.SIMPLE_ROUND_SPINNER );
    	progressHUD.show();
		String CheckDeliveryUrl=Constant.URL.CheckDeliveryURL+"&user_id="+userid+"&order_id="+orderId+"&token="+token;
		HTTPUtils.get(getActivity(), CheckDeliveryUrl, new VolleyListener() {
			...
			public void onResponse(String arg0) {
				...
				String succeed = status.getSucceed();
				if(succeed.equals("1")){
					ToastUtils.toast(getActivity(), "收货成功");
			...
			new Handler()
			{
				...
				public void handleMessage(Message msg)
				{
					if(mData.size()==0){
						...
						String AllOrderUrl=Constant.URL.MyOrderURL+"&user_id="+userid+"&status="+0+"&num="+size;
						HTTPUtils.get(getActivity(), AllOrderUrl, new VolleyListener() {
